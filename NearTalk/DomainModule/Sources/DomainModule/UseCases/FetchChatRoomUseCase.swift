...
public protocol FetchChatRoomUseCase {
...
public final class DefaultFetchChatRoomUseCase: FetchChatRoomUseCase {
    ...
    public func getGroupChatList() -> Observable<[GroupChatRoomListData]> {
    ...
    public func getDMChatList() -> Observable<[DMChatRoomListData]> {
        return self.chatRoom
            .asObservable()
    ...
    public func newObserveGroupChatList() -> Observable<[GroupChatRoomListData]> {
        self.newChatRoom
            .map { $0.filter { $0.roomType == "group" } }
            .map { $0.map { GroupChatRoomListData(data: $0) } }
    }
    
    public func newObserveDMChatList() -> Observable<[DMChatRoomListData]> {
        self.newChatRoom
            .map { $0.filter { $0.roomType == "dm" } }
            .map { $0.map { DMChatRoomListData(data: $0) } }
    }
    
    public func getGroupChatListWithCoordinates(southWest: NCLocation, northEast: NCLocation) -> Single<[ChatRoom]> {
        self.chatRoomListRepository.fetchChatRoomListWithCoordinates(southWest: southWest, northEast: northEast)
    }
    
    public func getUserChatRoomTickets() -> Single<[UserChatRoomTicket]> {
...
public enum FetchChatRoomUseCaseError: Error {
